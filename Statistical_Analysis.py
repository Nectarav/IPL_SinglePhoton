...
y = popt[0] + (popt[1] * np.log(x)) + (popt[2] * np.log(x) ** 2) + (popt[3] * np.log(x) ** 3)
...
FITLABEL = "0.386 + -0.0951ln(E) + 0.00949ln(E)^2 + -0.000480ln(E)^3"
...
PLOTTITLE = "Efficiency Curve of Sodium Iodide Scintillation Detector"
plt.title(PLOTTITLE)

# There are lots of legend locations, choose the one that looks best on your graph
plt.legend(loc = 7)
XLABEL = "Energy (keV)"
YLABEL = "Efficiency (photons/count)"
plt.xlabel(XLABEL)
plt.ylabel(YLABEL)
plt.show()
...
# If you need X error bars as well, you can use the same method to get those. Just make sure you add in X error
# wherever you do Y error.
#yErr = np.genfromtxt('YOURDATAHERE.csv', usecols=YCOLUMNERROR, skip_header=NUMBEROFHEADERS, delimiter=',')
...
plt.plot(xData, yData, "o", marker=".", color='blue')
# This one puts the calculated fit onto the plot as a line.
FITLABEL = "A + Bln(E) + Cln(E)^2 + Dln(E)^3"
plt.plot(x, y, color = 'blue', label=FITLABEL, linestyle = 'dotted')
# If you have error bars, you will want to uncomment this line below and use these error bars, changing color etc as
# appropriate
# plt.errorbar(xData, yData, yerr = yErr, color ='darkgreen', linestyle ='', capthick = 1, capsize = 1)
PLOTTITLE = "Efficiency Curve of Sodium Iodide Scintillation Detector"
plt.title(PLOTTITLE)
